...
void SpStud::kolneudstud(bool& exit)
{
	...
	else
	{
		...
		{
			if (massivik[a].size() % 5 == 0)
			{
				for (size_t b = 3; b < massivik[a].size(); b += 3)
				{
			...
			{
				for (size_t b = 3; b < massivik[a].size(); b += 2)
				{
					if (massivik[a][b].find(name0) != -1)
					{
						kolNed++;
...
void SpStud::poisk(bool& exit)
{
	...
	else
	{
		...
		string name = "";
		cout << "Введите ФИО студента(в качестве разделителя использовать <_>):";
		cin.ignore();
		getline(cin, name);
